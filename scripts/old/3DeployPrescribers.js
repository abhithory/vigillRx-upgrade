...
async function main() {
    ...
    const CreatePrescribers = await Registrar.createPrescriber(address,npi);
    const createPrescribers = await CreatePrescribers.wait();
    console.log(` Prescriber Address: ${createPrescribers.events[0].args[0]}`);
}
